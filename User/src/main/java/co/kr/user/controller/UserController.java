 ...
 * 마이페이지 조회, 상세 정보 조회, 회원 탈퇴, 정보 수정 등의 기능을 제공합니다.
 */
@Validated // 요청 파라미터나 바디의 데이터 유효성 검증을 활성화하는 어노테이션입니다.
@RestController // 이 클래스가 RESTful API 컨트롤러임을 나타내며, 반환값은 자동으로 JSON 형식으로 변환됩니다.
@RequiredArgsConstructor // final이 붙은 필드에 대해 생성자를 자동으로 생성해주어 의존성을 주입받게 합니다.
@RequestMapping("/users") // 이 클래스 내의 모든 메서드는 "/users"로 시작하는 URL 경로에 매핑됩니다.
public class UserController {
    // 회원 관련 비즈니스 로직을 처리하는 서비스 객체입니다.
    private final UserService userService;

    /**
     * 내 정보 조회 API (기본 정보)
     * 사용자의 기본적인 정보를 조회합니다.
     * * @param userIdx HTTP 헤더 "X-USERS-IDX"에서 추출한 사용자의 고유 식별자 (Gateway 등에서 주입됨)
     * @return ResponseEntity<UserDTO> 사용자 기본 정보(UserDTO)를 포함한 응답 객체
     */
    @PostMapping("/my")
    public ResponseEntity<UserDTO> my(@RequestHeader ("X-USERS-IDX") Long userIdx) {
        // userService를 통해 사용자 정보를 조회하고, HTTP 200 OK 상태와 함께 반환합니다.
     ...
     * @return ResponseEntity<UserProfileDTO> 사용자 상세 프로필 정보(UserProfileDTO)를 포함한 응답 객체
     */
    @PostMapping("/my/details")
    public ResponseEntity<UserProfileDTO> myDetails(@RequestHeader ("X-USERS-IDX") Long userIdx) {
        // userService를 통해 상세 프로필 정보를 조회하고 반환합니다.
        return ResponseEntity.ok(userService.myDetails(userIdx));
    }

    /**
     * 회원 탈퇴 요청 API (1단계)
     * 회원 탈퇴 절차를 시작하는 요청입니다. (예: 탈퇴 확인을 위한 정보 반환 또는 인증 절차 시작)
     * * @param userIdx HTTP 헤더 "X-USERS-IDX"에서 추출한 사용자의 고유 식별자
     * @return ResponseEntity<UserDeleteDTO> 탈퇴 요청에 대한 처리 결과 또는 필요한 정보를 담은 객체
     */
    @PostMapping("/my/delete")
    public ResponseEntity<UserDeleteDTO> myDelete(@RequestHeader ("X-USERS-IDX") Long userIdx) {
        // userService를 통해 탈퇴 요청 로직을 수행하고 결과를 반환합니다.
        return ResponseEntity.ok(userService.myDelete(userIdx));
    }

    /**
     * 회원 탈퇴 확정 API (2단계)
     * 인증 코드를 받아 검증한 후, 최종적으로 회원을 탈퇴 처리합니다.
     * * @param userIdx HTTP 헤더 "X-USERS-IDX"에서 추출한 사용자의 고유 식별자
     * @param userDeleteSecondStepReq HTTP Body에 포함된 탈퇴 확인을 위한 데이터 (인증 코드 등)
     * @Valid 어노테이션을 통해 데이터의 유효성을 검증합니다.
     * @return ResponseEntity<String> 탈퇴 처리 결과 메시지를 포함한 응답 객체
     */
    ...
    public ResponseEntity<UserAmendReq> myamend(@RequestHeader ("X-USERS-IDX") Long userIdx,
        ...
        // userService를 통해 사용자 정보를 수정하고, 수정된 데이터를 반환합니다.
