...
public class ViewCountService {
    ...
    // Redis를 통한 조회수 증가
    public Long increaseViewCountProduct(Long noticeIdx){

        // 조회수 증가
        Long viewCount = stringRedisTemplate.opsForValue().increment(viewCountKey + noticeIdx);

        // 숫자 그대로 저장하면 나중에 뺄 때 타입 오류 생김.
        // Jackson이 큰 수냐 작은 수냐에 따라 Integer을 뱉기도 하고 Long을 뱉기도 함.
        stringRedisTemplate.opsForSet().add(dirtyKey, String.valueOf(noticeIdx));
